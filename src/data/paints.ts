import type { Paint, PaintRange } from '@/types';

// Static reference catalog of paints. Kept client-side (reference data) so we
// only need to persist the user's own collection/wishlist in Supabase.
...
function buildCatalog(): Paint[] {
  const citadel: Paint[] = CITADEL.map(([name, range, hex, metallic]) => ({
    id: `citadel-${name.toLowerCase().replace(/[^a-z0-9]/g, '-')}`,
    name,
    brand: 'Citadel',
    range,
    hexColor: hex,
    isMetallic: metallic === 1,
  }));

  const vallejo: Paint[] = VALLEJO.map(([ref, name, range, hex, metallic]) => ({
    id: `vallejo-${ref}`,
    name: `${ref} ${name}`,
    brand: 'Vallejo',
    range,
  ...
  return [...citadel, ...vallejo];
}

export const PAINT_CATALOG: Paint[] = buildCatalog();

export const PAINT_CATALOG_BY_ID: Map<string, Paint> = new Map(
  PAINT_CATALOG.map((p) => [p.id, p]),
);
